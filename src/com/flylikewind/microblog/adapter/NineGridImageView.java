...
public class NineGridImageView<T> extends ViewGroup {
	...
	public NineGridImageView(Context context, AttributeSet attrs) {
		...
		this.mGap = (int) typedArray.getDimension(
				R.styleable.NineGridImageView_imgGap, 0);
		this.mSingleImgSize = typedArray.getDimensionPixelSize(
				R.styleable.NineGridImageView_singleImgSize, -1);
		this.mShowStyle = typedArray.getInt(
				R.styleable.NineGridImageView_showStyle, STYLE_GRID);
		this.mMaxSize = typedArray.getInt(
				R.styleable.NineGridImageView_maxSize, 9);
		typedArray.recycle();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mImgDataList != null && mImgDataList.size() > 0) {
			if (mImgDataList.size() == 1 && mSingleImgSize != -1) {
				mGridSize = mSingleImgSize > totalWidth ? totalWidth
						: mSingleImgSize;
			} else {
				mImageViewList.get(0).setScaleType(
						ImageView.ScaleType.CENTER_CROP);
				mGridSize = (totalWidth - mGap * (mColumnCount - 1))
						/ mColumnCount;
			}
			...
			if (mAdapter != null) {
				...
				imageView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						mAdapter.onItemImageClick(getContext(), position,
								mImgDataList);
					}
				});
				return imageView;
