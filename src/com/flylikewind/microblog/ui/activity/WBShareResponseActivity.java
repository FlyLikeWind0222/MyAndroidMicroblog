...
/**
 * 该类演示了第三方应用如何通过微博客户端分享文字、图片、视频、音乐等。 执行流程： 从微博->本应用->微博
 * <p>
 * <b>该类与 {@link WBShareActivity} 类的界面及流程大致一样，请注意区分。</b>
 * </p>
 * 
 * @author SINA
 * @since 2013-11-01
 */
public class WBShareResponseActivity extends Activity implements
	...
	public void onRequest(BaseRequest baseRequest) {
		...
		int resId = (mBaseRequest != null) ? R.string.weibosdk_demo_toast_share_response_args_success
	...
	public void onClick(View v) {
		...
		if (R.id.share_to_btn == v.getId()) {
			...
			responseMessage(mTextCheckbox.isChecked(),
					mImageCheckbox.isChecked(), mShareWebPageView.isChecked(),
					mShareMusicView.isChecked(), mShareVideoView.isChecked(),
	...
	private void initViews() {
		...
		mShareMusicView.initWithRes(R.string.weibosdk_demo_share_music_title,
		...
		mShareVideoView.initWithRes(R.string.weibosdk_demo_share_video_title,
		...
		mShareVoiceView.initWithRes(R.string.weibosdk_demo_share_voice_title,
	...
	private void responseMessage(boolean hasText, boolean hasImage,
		...
		if (mShareWeiboAPI.isWeiboAppSupportAPI()) {
			...
			if (supportApi >= 10351 /* ApiUtils.BUILD_INT_VER_2_2 */) {
				responseMultiMessage(hasText, hasImage, hasWebpage, hasMusic,
						hasVideo, hasVoice);
			} else {
				responseSingleMessage(hasText, hasImage, hasWebpage, hasMusic,
						hasVideo/* , hasVoice */);
			}
		} else {
	...
	/**
	 * 第三方应用响应微博客户端的请求，提供需要分享的数据。 注意：当
	 ...
	 *            分享的内容是否有声音
	 */
	private void responseMultiMessage(boolean hasText, boolean hasImage,
			boolean hasWebpage, boolean hasMusic, boolean hasVideo,
			boolean hasVoice) {
		// 1. 初始化微博的分享消息
