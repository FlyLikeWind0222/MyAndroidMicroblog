...
public class WBOpenAPIActivity extends Activity implements OnItemClickListener {
	...
	private static final String DEST_ACTIVITY_PACKAGE_NAME = "com.flylikewind.microblog.ui.openapi";
	/** 该 MAP 用于存放 OpenAPI 名称以及对应的 DEMO Activity 名 */
	private static final LinkedHashMap<String, String> sAPIList = new LinkedHashMap<String, String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mApiListView.setAdapter(new ArrayAdapter<String>(this,
				android.R.layout.simple_list_item_1, getAPINameList()));
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (view instanceof TextView) {
			String className = sAPIList.get(((TextView) view).getText()
					.toString());
			if ("refreshtoken".equals(className)) {
				...
				intent.setClassName(getPackageName(),
						DEST_ACTIVITY_PACKAGE_NAME + "." + className);
				try {
	...
	private void refreshTokenRequest() {
		Oauth2AccessToken token = AccessTokenKeeper
				.readAccessToken(WBOpenAPIActivity.this);
		RefreshTokenApi.create(getApplicationContext()).refreshToken(
				Constants.APP_KEY, token.getRefreshToken(),
				new RequestListener() {

					@Override
					public void onWeiboException(WeiboException arg0) {
						Toast.makeText(WBOpenAPIActivity.this,
